...
|                 |              |            |         |                                 |          |    |          |            |        |
| synch. variable | index_x size | neigh_size | *neighs | *column size for each index row | index[0] | .. | index[N] | index[N+1] | unused |
|                 |              |            |  array  |               array             |   row 0  |    |   row N  |   row N+1  |        |
|                 |              |            |         |                                 |__________|____|__________|____________|        |
|      int        |    size_t    |   size_t   | DestID_ |               size_t            |         DestID_ x DestID_             |        |
|_________________|______________|____________|_________|_________________________________|_______________________________________|________|
```
