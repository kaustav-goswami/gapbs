...
class BuilderBase {
 ...
 public:
  ...
  // neighs.
  size_t index_x, index_y, neighs_x, neighs_y;
  ...
  void SquishCSR(const CSRGraph<NodeID_, DestID_, invert> &g, bool transpose,
    ...
    // index_neighs = new size_t;
    // *index_neighs = sq_offsets[g.num_nodes()]; // (size_t)diffs.size();
  ...
  CSRGraph<NodeID_, DestID_, invert> SquishGraph(
    ...
    if (host_id == 0) {
      ...
      if (g.directed()) {
        ...
        std::cout << this->index_x << " " << index_x << std::endl;
        index_x = this->index_x;
        index_y = this->index_x;
        std::cout << this->neighs_x << " " << index_out_neighs << std::endl;
        // exit(-1);
        index_out_neighs = this->neighs_x; //  = 20992;
        std::cout << "master x: " << this->index_x << " y: " << this->index_x << " neighs: " << this->neighs_x << std::endl;
        return CSRGraph<NodeID_, DestID_, invert>(g.num_nodes(), &out_index,
                                    this->index_x, this->index_x, &out_neighs,
                                    index_out_neighs, host_id, false); // false);
      }
    }
    else {
  ...
  */
  void MakeCSR(const EdgeList &el, bool transpose, DestID_*** index,
    ...
    // *neighs is a 2d. matrix?
    *neighs = new DestID_[offsets[num_nodes_]];

    this->index_x = offsets.size();
    // this->index_y 

    // The x dimension of neighs is neighs_x = offsets[num_nodes_]
    *index = CSRGraph<NodeID_, DestID_>::GenIndex(offsets, *neighs);
    #pragma omp parallel for
    for (auto it = el.begin(); it < el.end(); it++) {
      Edge e = *it;
  ...
  // kg: This is what the worker does!
  CSRGraph<NodeID_, DestID_, invert> ReadPreviouslyCreatedGraph() {

  }

  CSRGraph<NodeID_, DestID_, invert> MakeGraphFromEL(EdgeList &el, int host_id) {
    DestID_ **index = nullptr, **inv_index = nullptr;
    DestID_ *neighs = nullptr, *inv_neighs = nullptr;

    // TODO: Back these data structures via the backing store instead of
    // allocting memory separately.

    // kg: Checkout the params.
    // So the el will be deleted after this method is executed (pretty much).
    std::cout << "info: num_nodes_ = " << num_nodes_ <<
            "; needs_weights_ = " << needs_weights_ <<
            "; in_place_ = " << in_place_ <<
            "; symm = " << symmetrize_ << "; invert = " << invert << std::endl;
    if (host_id != 0) {
      if (num_nodes_ == -1)
    ...
    if (symmetrize_) {
      ...
      // kg: This is what is _NOT_ called for the given method.
      return CSRGraph<NodeID_, DestID_, invert>(num_nodes_, index, neighs,
  ...
  CSRGraph<NodeID_, DestID_, invert> MakeGraph(int host_id) {
    ...
    {  // extra scope to trigger earlier deletion of el (save memory)
      EdgeList el;
      std::cout << cli_.uniform() << std::endl;
      ...
      if (host_id == 0) {
        ...
        // The graph object g should have all the information.
        // g = readPreviouslyCreatedGraph();
        exit(-1);
